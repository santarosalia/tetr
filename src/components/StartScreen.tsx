    ...
    <div className="w-full h-full bg-gradient-to-b from-blue-900 via-purple-900 to-indigo-900 flex flex-col justify-center items-center">
          ...
          {['I', 'O', 'T', 'S', 'Z', 'J', 'L'].map((type, index) => (
            <div
              ...
              style={{
                ...
                backgroundColor: type === 'I' ? 'cyan' : type === 'O' ? 'yellow' : type === 'T' ? 'purple' : type === 'S' ? 'green' : type === 'Z' ? 'red' : type === 'J' ? 'blue' : type === 'L' ? 'orange' : 'white'
              }}
            />
          ))}
        </div>
      ...
      <div className="text-center text-white mb-8">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p><span className="font-bold">← →</span> 이동</p>
            <p><span className="font-bold">↓</span> 빠른 하강</p>
          </div>
          <div>
            <p><span className="font-bold">↑</span> 회전</p>
            <p><span className="font-bold">스페이스</span> 즉시 하강</p>
          </div>
        </div>
      </div>
