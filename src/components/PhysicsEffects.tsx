...
export interface PhysicsEffectsRef {
  ...
  createPieceDropEffect: (pieceX: number, pieceY: number, pieceType: string, shape: boolean[][]) => void;
}
...
export const PhysicsEffects = forwardRef<PhysicsEffectsRef, PhysicsEffectsProps>(({ 
  ...
  useEffect(() => {
    ...
    if (engine.render && engine.render.canvas) {
      ...
      engine.render.canvas.addEventListener('keydown', (e) => {
        e.preventDefault();
        e.stopPropagation();
        return false;
      }, true);
  ...
  const createBlock = (x: number, y: number, color: string, size: number = BLOCK_SIZE - 2) => {
  ...
  const createPieceDropEffect = (pieceX: number, pieceY: number, pieceType: string, shape: boolean[][]) => {
    if (!engineRef.current) return;
